...
void F_move_edge() {
...

    {
        ...
        // 等价于
        // std::move(map2) = std::move(map1);  // 这里虽然std::move(map2)了，但是map2实际上还是一个左值
        // map1 = map2;

        // map1 = std::move(map2) = std::move(map2) = std::move(map1);  // 修改这行测试代码

        std::move(map2) = std::move(map1);
        std::move(map2) = std::move(map2);
        std::move(map1) = std::move(map2);

        answer1 = std::move(answer2) = std::move(answer2) = std::move(answer1);
        VERIFY_TRUE(check_map_equal(map1, answer1), __LINE__);

        map2 = std::move(map1);  /** 由于前面在最后move了map2，因此map2的资源已经不可用了
         * 此时直接访问map2的资源就会导致未定义行为，因此这里的测试是不合理的。
         * 正确的测试应该是在测试之前重新把map2的资源从map1上move回来，然后再对map2进行测试。
         */ 
        VERIFY_TRUE(check_map_equal(map2, answer2), __LINE__);
        map1 = std::move(map2);
        ...
        map2 = std::move(map1);  /* 与前面同理 */
        VERIFY_TRUE(check_map_equal(map2, answer2), __LINE__);
        map1 = std::move(map2); /* 重新move回来 */
