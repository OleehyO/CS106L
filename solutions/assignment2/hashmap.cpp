...
std::pair<typename HashMap<K, M, H>::iterator, bool> HashMap<K, M, H>::insert(const value_type& value) {
    ...
    node_ptr temp{new node{value, _buckets_array[index]}};
...
typename HashMap<K, M, H>::node_pair HashMap<K, M, H>::find_node(const K& key) const {
    size_t index = _hash_function(key) % bucket_count();
    node_ptr curr = _buckets_array[index];
    node_ptr prev = nullptr; // if first node is the key, return {nullptr, front}
    while (curr != nullptr) {
...
template <typename K, typename M, typename H>
typename HashMap<K, M, H>::iterator HashMap<K, M, H>::make_iterator(node_ptr curr) const {
    if (curr == nullptr) {
        return {const_cast<bucket_array_type*>(&_buckets_array), curr, bucket_count()};
    }
    size_t index = _hash_function(curr->value.first) % bucket_count();
    return {const_cast<bucket_array_type*>(&_buckets_array), curr, index};
}
...
void HashMap<K, M, H>::debug() {
    ...
    for (size_t i = 0; i < bucket_count(); ++i) {
        ...
        node_ptr curr = _buckets_array[i];
        while (curr != nullptr) {
...
void HashMap<K, M, H>::rehash(size_t new_bucket_count) {
    ...
    std::vector<node_ptr> new_buckets_array(new_bucket_count, nullptr);
...
HashMap<K, M, H>::HashMap(const HashMap<K, M, H>& other): 
    HashMap(other.begin(), other.end(), other.bucket_count(), other._hash_function)
{
    ;
}
...
HashMap<K, M, H>& HashMap<K, M, H>::operator=(HashMap<K, M, H>&& other) {
    if (this != &other) {
        ...
        _size = std::move(other._size);
        _hash_function = std::move(other._hash_function);
        _buckets_array = std::move(other._buckets_array);  // 直接使用vector提供的移动语义的赋值
    }
    return *this;
}
